...
2. Claude: Codexの下書きに書かれた記事URLを全件curlで機械確認する（200 OK確認。
           Codexの「確認できた」報告は鵜呑みにしない）。続けて各記事URLの生HTMLを
           curlで取得し、`<meta property="og:image">`をgrepで抜いてサムネイル画像URLを
           取得し、その画像URL自体も200 OKかを確認する（2026-09-17検証：この方式で
           Codexがfavicon・404画像に逃げた5件を一発で正しい画像に置き換えられた）。
