...
- **一覧カードの`bullets`（`articles.json`）: 必ず3件。** 記事本文を読まなくても
  概要がつかめる、キャッチーで短い要点3つ。「結論」の全項目から重要な3つを
  抜き出すのではなく、カード表示に適した短い文で**別途書き直す**
- **記事詳細ページの「結論」（HTML内`<ul class="points">`）: 篇数無制限。** 詳細は
  下記参照

`automation/build-batch-drafts.js`を使う場合、`articles-batch*.json`に
`bullets`（結論用、篇数無制限）と`cardBullets`（一覧カード用、**必ず3件**）を
別フィールドとして持たせる。`cardBullets`が3件でなければスクリプトがエラーで
止まるようになっている（2026-09-16のレイアウト崩れ事故を受けて追加した検証）。
