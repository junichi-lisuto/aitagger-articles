...
function Invoke-ResearchRun {
    ...
    Write-SummaryLine "aitagger-articles 自動リサーチを開始します(PID: $PID / 目標本数: ${minCount}〜${maxCount}本)"
    ...
    try {
        ...
        Get-Content $renderedPromptPath -Raw -Encoding utf8 | & claude -p --output-format stream-json --verbose --allowedTools "Bash,Read,Write,Edit,Glob,Grep,WebFetch,WebSearch" 2>$stdErrPath | ForEach-Object {
            $line = $_.ToString()
...
if ($retryTime) {
    $waitMinutes = [math]::Ceiling(((New-TimeSpan -Start (Get-Date) -End $retryTime).TotalMinutes))
    Write-Host ""
    Write-Host "利用上限のため失敗しました。リセット時刻($retryTime)まで待って自動リトライします(約${waitMinutes}分後)。"
    ...
    $delaySeconds = [math]::Max(60, [int]((New-TimeSpan -Start (Get-Date) -End $retryTime).TotalSeconds))
    Start-Sleep -Seconds $delaySeconds

    Write-Host "リトライを開始します..."
